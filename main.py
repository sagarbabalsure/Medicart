...
def buyer_login():
	...
	return render_template("buyer_login.html",error=error,form=form)
...
def buyer_register():
	...
	return render_template("buyer_register.html",form=form)
...
def seller_login():
	...
	if request.method == 'POST' and form.validate_on_submit():
		...
		if data:
			...
			error="invalid username or password"
	return render_template("seller_login.html",error=error,form=form)
...
def profile():
	if 'username' in session:
		...
		if mode:
			data = cur.execute("SELECT * from buyer_register where username=%s",(username))
			data = cur.fetchone()
			return render_template("buyer_profile.html",data=data)
		else:
			data = cur.execute("SELECT * from seller_register where username=%s",(username))
			data = cur.fetchone()
			return render_template("seller_profile.html",data=data)
	else:
		return render_template('index.html')
...
def homepage():
	if 'username' in session:
		...
		if mode:
			...
			data = cur.fetchall()
			return render_template('homepage_data.html',data=data)
		else:
...
def edit_profile():
	...
	if 'username' in session:
		...
		if mode:
			...
			return render_template("edit_buyer_profile.html",form=form,data=data)
		else:
			...
			if request.method == 'POST' and form.validate_on_submit():
				...
				flash("Your profile is updated")
				return redirect(url_for('profile'))
			return render_template("edit_seller_profile.html",form=form,data=data)
	return render_template('index.html')
